...
def get_config():
    ...
    try:
        if not os.path.exists(CONFIG_FILE_PATH):
            ...
            raise HTTPException(status_code=404, detail="Configuration file not found. Ensure tacacs_ng service is running to create the default config.")
        ...
        raise HTTPException(status_code=500, detail="Unexpected error: Configuration file disappeared.")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error reading config: {e}")

@router.post("/config/save")
def save_config(
    # Sử dụng Body(..., embed=True) để lấy giá trị chuỗi từ key 'new_content' trong JSON body
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error writing config: {e}. Check volume permissions.")
...
def get_auth_logs(lines: int = 20):
    ...
    if lines <= 0:
        raise HTTPException(status_code=400, detail="The 'lines' parameter must be a positive integer.")
        
    try:
        if not os.path.exists(LOG_FILE_PATH):
            return {"logs": [], "message": "Authentication log file does not exist yet. Run some TACACS+ tests to generate logs."}
